...
$list=Yii::app()->functions->getIngredientsList($merchant_id);
?>
...
<?php echo CHtml::hiddenField('table','ingredients')?>
<?php echo CHtml::hiddenField('whereid','ingredients_id')?>
...
<?php if (is_array($list) && count($list)>=1):?>
  ...
  <?php foreach ($list as $val):?>
   <li class="uk-panel uk-panel-box" style="list-style:none;margin-bottom:5px;">
    <?php echo CHtml::hiddenField('sort_field[]',$val['ingredients_id'])?>
    <i class="fa fa-arrows-alt"></i>
    <?php echo ucwords($val['ingredients_name'])?>
   </li>
  <?php endforeach;?>
  </ul>
<?php else :?>
